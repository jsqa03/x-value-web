...
const ExclusivityPanel = dynamic(
  () => import("./xbank/ExclusivityPanel"),
  {
    ssr: false,
    loading: () => (
      <div
...
// ─── Fade-up reveal — respects prefers-reduced-motion ────────────────────────
function FadeUp({
  children,
  delay = 0,
  className = "",
...
// ─── Credit card mockup — GPU-accelerated, reduced-motion aware ───────────────
function CreditCardMockup({ companyName }: { companyName: string }) {
  ...
  return (
    <motion.div
      ...
      style={{ maxWidth: 440, perspective: prefersReducedMotion ? undefined : 1400 }}
      initial={{ opacity: 0, rotateY: prefersReducedMotion ? 0 : -18, y: prefersReducedMotion ? 0 : 48 }}
      whileInView={{ opacity: 1, rotateY: 0, y: 0 }}
      viewport={{ once: true }}
      transition={
        prefersReducedMotion
          ? { duration: 0.3 }
          : { duration: 1.1, ease: [0.22, 1, 0.36, 1], delay: 0.1 }
      }
    >
      {/* Ambient outer glow — GPU-composited to avoid repaint */}
      <div
        className="absolute inset-0 rounded-3xl pointer-events-none"
        style={{
          background:
            "radial-gradient(ellipse at 55% 45%, rgba(245,158,11,0.20) 0%, rgba(234,88,12,0.10) 45%, transparent 75%)",
          filter: "blur(36px)",
      ...
      <motion.div
        ...
        style={{
          ...
          // Simplified shadows: removed the expensive 100px radial glow
          boxShadow:
            "0 0 0 0.5px rgba(245,158,11,0.14), 0 24px 56px rgba(0,0,0,0.72)",
          // GPU layer — prevents Safari layout thrashing on hover animation
          willChange: "transform",
          backfaceVisibility: "hidden",
          WebkitBackfaceVisibility: "hidden",
        }}
        whileHover={
          prefersReducedMotion ? {} : { scale: 1.025, rotateX: -3, rotateY: 5 }
        }
        transition={
          prefersReducedMotion
            ? {}
            : { type: "spring", stiffness: 160, damping: 32 }
        }
      >
        ...
        <div className="relative z-10 h-full flex flex-col justify-between p-6 sm:p-8">
          ...
          <div className="flex items-start justify-between">
            ...
            {/* Contactless NFC */}
            <div className="flex flex-col items-center justify-center" style={{ opacity: 0.38 }}>
...
// ─── CardPersonalizer — isolated state to prevent full-page re-renders ────────
// Exposes getCompanyName() via ref so the waitlist form can read it on submit
// without needing companyName in the parent's state.
interface CardPersonalizerHandle {
  getCompanyName: () => string;
}

const CardPersonalizer = forwardRef<CardPersonalizerHandle>(
  function CardPersonalizer(_, ref) {
          ...
          >
            ...
            <div className="relative">
              <input
                type="text"
                value={companyName}
                onChange={(e) => setCompanyName(e.target.value.slice(0, 26))}
...
export default function XBankLanding({ stats }: Props) {
  ...
  const cardRef = useRef<CardPersonalizerHandle>(null);
  const prefersReducedMotion = useReducedMotion();
  ...
  function handleWaitlist(e: React.FormEvent) {
    ...
    const companyName = cardRef.current?.getCompanyName() ?? "";
    setServerError("");
  ...
  return (
    ...
    >
      ...
      {/* ── Ambient layer — GPU-composited blurs ──────────────────────────── */}
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-6 flex flex-col gap-24">
        ...
        <div className="flex flex-col items-center text-center gap-7">
          ...
          >
            ...
            >
              <span
                className={`w-1.5 h-1.5 rounded-full shrink-0 ${prefersReducedMotion ? "" : "animate-pulse"}`}
          ...
          <motion.h1
            ...
            transition={
              prefersReducedMotion
                ? { duration: 0.2 }
                : { duration: 0.85, ease: [0.22, 1, 0.36, 1], delay: 0.1 }
            }
          >
          ...
          <motion.p
            ...
            style={{ fontSize: "clamp(15px, 1.9vw, 19px)", color: "rgba(255,255,255,0.5)" }}
            initial={{ opacity: 0, y: prefersReducedMotion ? 0 : 16 }}
            animate={{ opacity: 1, y: 0 }}
            transition={
              prefersReducedMotion
                ? { duration: 0.2 }
                : { duration: 0.75, delay: 0.22 }
            }
          >
          ...
          <motion.div
            ...
            transition={
              prefersReducedMotion
                ? { duration: 0.2 }
                : { duration: 0.6, delay: 0.36 }
            }
          >
            {HERO_PILLS.map((p) => (
              <span
                key={p.label}
                className="group inline-flex items-center gap-2.5 px-5 py-2.5 rounded-[20px] text-sm font-medium transition-colors duration-300 cursor-default select-none"
                style={{
        ...
        {/* ══════════════════════════════════════════════════════════════════
            CARD + PERSONALIZACIÓN + WALLETS
            CardPersonalizer manages companyName internally — typing in the
            input only re-renders this sub-tree, not the entire landing page.
        ══════════════════════════════════════════════════════════════════ */}
        <div className="flex flex-col lg:flex-row items-center gap-14 lg:gap-16">
          <CardPersonalizer ref={cardRef} />
        ...
        {/* ══════════════════════════════════════════════════════════════════
            BENEFITS CAROUSEL — dynamically imported, below the fold
        ══════════════════════════════════════════════════════════════════ */}
        <FadeUp>
          <BenefitsCarousel />
        </FadeUp>
        ...
        {/* ══════════════════════════════════════════════════════════════════
            EXCLUSIVITY PANEL — dynamically imported, below the fold
        ══════════════════════════════════════════════════════════════════ */}
        <FadeUp>
          <ExclusivityPanel bullets={exclusivityBullets} />
        </FadeUp>
        ...
        <FadeUp>
          <div className="flex flex-col items-center text-center gap-7">
            ...
            <p
              ...
              style={{ fontSize: "clamp(13px, 1.5vw, 15px)", color: "rgba(255,255,255,0.42)" }}
            >
                  ...
                  <div className="flex flex-col sm:flex-row gap-2.5">
                    <input
                      ...
                      onFocus={(e) => {
                        ...
                        e.currentTarget.style.boxShadow = "0 0 0 3px rgba(245,158,11,0.06)";
                      }}
                      onBlur={(e) => {
                        e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)";
                        e.currentTarget.style.boxShadow = "none";
                    ...
                    <button
                      ...
                      style={{
                        ...
                        boxShadow:
                          "0 0 0 1px rgba(245,158,11,0.3), 0 8px 20px rgba(245,158,11,0.18)",
                      }}
                      onMouseEnter={(e) => {
                        (e.currentTarget as HTMLButtonElement).style.boxShadow =
                          "0 0 0 1px rgba(245,158,11,0.5), 0 12px 32px rgba(245,158,11,0.3)";
            ...
            >
              <div className="flex -space-x-2 shrink-0">
                {(["#f59e0b", "#ea580c", "#a78bfa", "#22c55e", "#38bdf8"] as const).map((c) => (
                  <div
                    key={c}
              ...
              <span className="text-xs" style={{ color: "rgba(255,255,255,0.32)" }}>
                <strong style={{ color: "rgba(255,255,255,0.55)" }}>+{waitlistCount}</strong>{" "}
                empresas ya en lista de espera
              </span>
            </div>
